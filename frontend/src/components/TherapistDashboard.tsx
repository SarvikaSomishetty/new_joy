...
const preparePuzzleData = (session: Session) => {
  ...
  const getEmotionColor = (emotion: string | undefined | null): string => {
    if (!emotion) return '#9E9E9E'; // Handle undefined/null
      const emotionColors: { [key: string]: string } = {
      happy: '#4CAF50',
      happiness: '#4CAF50', // Added happiness
      sad: '#5C6BC0',
      anger: '#FF0000',
      fear: '#FF9800',
      neutral: '#9E9E9E',
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPasswordChangeMessage(data.error || data.message || 'Failed to change password');
      }
    } catch (err) {
